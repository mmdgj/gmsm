...
func WritePrivateKeyToPemFile(FileName string, key *sm2.PrivateKey, pwd []byte) (bool, error) {
	var block *pem.Block
...
func WritePublicKeyToPemFile(FileName string, key *sm2.PublicKey, _ []byte) (bool, error) {
	der, err := MarshalSm2PublicKey(key)
	if err != nil {
		return false, err
	}
